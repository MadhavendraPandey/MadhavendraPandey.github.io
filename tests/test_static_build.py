...
class StaticBuildTests(unittest.TestCase):
    def test_project_site_export(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as folder:
            output = build(Path(folder) / "site", "/portfolio/")
            ...
            self.assertIn('src="/portfolio/static/js/pet.js', home)
            self.assertIn('href="/portfolio/about/"', home)
            self.assertIn('href="/portfolio/work/"', home)
            self.assertIn('href="/portfolio/projects/academic-inbox/"', home)
            self.assertIn('id="work"', (output / "work/index.html").read_text(encoding="utf-8"))

    def test_url_prefix_and_root_site(self):
        self.assertEqual(static_url("/#work", "/portfolio/"), "/portfolio/work/")
        self.assertEqual(static_url("/static/favicon.svg", "/"), "/static/favicon.svg")
        self.assertEqual(static_url("//cdn.example.com/file.js", "/portfolio/"),
